 ...
 */
Direction nodeDirection(Node *node) // todo check with lab
{
 ...
 * @param node: node its color required.
 * @return: RED if the color of node is RED, BLACK otherwise.
 */
...
{
    return nodeDirection(node) == RIGHT ? node->parent->left : node->parent->right;
}
...
Node * recolor(Node *node)
{
    while (node->parent != NULL)
    {
        ...
        if (getColor(getSibling(node->parent)) == BLACK)
        {
            return node;
        }
...
/**
 * handle case where one of the Nodes in an RBTree is a double black parent.
 * @param tree: a RBTree that contain a double black Node.
 * @param parent: the parent of the double black Node in tree.
 * @param direction: the direction from parent to to its double black child.
 */
void doubleBlackNode(RBTree *tree, Node *parent, Direction direction)
{
    if (parent == NULL)
    {
        return;
    }
    Node * dbSibling = direction == LEFT ? parent->right : parent->left;
    if (dbSibling->color == BLACK && getColor(dbSibling->left) == BLACK && getColor(dbSibling->right) == BLACK)
    {
        dbSibling->color = RED;
        ...
        {
            doubleBlackNode(tree, parent->parent, nodeDirection(parent));
        }
        return;
    }
    if (getColor(dbSibling) == RED && getColor(parent->parent) == BLACK) {
        switchColors(dbSibling, parent);
        rotate(tree, parent, dbSibling, direction);
        doubleBlackNode(tree, parent, direction);
        return;
    }
    Node * dbNearNephew = direction == LEFT ? dbSibling->left : dbSibling->right;
    Node * dbDistantNephew = direction == LEFT ? dbSibling->right : dbSibling->left;
    switch (getColor(dbSibling) == BLACK && getColor(dbDistantNephew) == BLACK)
    {
        case true:
        {
            switchColors(dbSibling, dbNearNephew);
            rotate(tree, dbSibling, dbNearNephew, !direction);
            dbDistantNephew = dbSibling;
            dbSibling = dbNearNephew;
        }
        case false:
        {
            switchColors(dbSibling, parent);
            rotate(tree, parent, dbSibling, direction);
            if (getColor(dbDistantNephew) == RED)
            {
                dbDistantNephew->color = BLACK;
            }
        }
    }
}
...
void deleteNode(RBTree *tree, Node * node)
{
    ...
    Node * child = successor->left == NULL ? successor->right : successor->left;
    if (getColor(successor) == RED)
    {
